    ...
    def add_steps_and_auto_finish(
        ...
        for env_idx, (act, ob, rew, done, info) in zip_iter:
            if done:
                ...
                real_ob = info["terminal_observation"]['observation']                
            else:                
                real_ob = ob

            tmp_info = {}
            if "rollout" in info:                                
                tmp_info['rollout'] = info['rollout']
                tmp_info['rollout']['obs'] = [tmp_obs['observation'] for tmp_obs in info['rollout']['obs']]
    ...
    *,
    ...
    Args:
        ...
        sample_until: A function determining the termination condition.
            ...
            Most users will want to use one of `min_episodes` o r `min_timesteps`.
    ...
    obs = venv.reset()            
    for env_idx, ob in enumerate(obs['observation']):
    ...
    for trajectory in trajectories:
        ...
        # extra 1 for the end        
        exp_obs = (n_steps + 1,) + venv.observation_space['observation'].shape
