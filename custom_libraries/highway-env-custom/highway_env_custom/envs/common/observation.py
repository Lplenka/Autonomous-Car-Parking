...
from highway_env_custom.envs.common.finite_mdp import compute_ttc_grid
from highway_env_custom.envs.common.graphics import EnvViewer
from highway_env_custom.road.lane import AbstractLane
from highway_env_custom.utils import distance_to_circle, Vector
from highway_env_custom.vehicle.controller import MDPVehicle
from highway_env_custom.vehicle.kinematics import Vehicle

if TYPE_CHECKING:
    from highway_env_custom.envs.common.abstract import AbstractEnv
...
class KinematicsGoalObservation(KinematicObservation):
    ...
    def space(self) -> spaces.Space:
        try:
            obs = self.observe()
            return spaces.Box(-np.inf, np.inf, shape=obs.shape, dtype=np.float64)
        except AttributeError:
            return spaces.Space()

    def space_original(self) -> spaces.Space:
        try:
            obs = self.observe()
    ...
    def observe_original(self) -> Dict[str, np.ndarray]:
        if not self.observer_vehicle:
            return {
